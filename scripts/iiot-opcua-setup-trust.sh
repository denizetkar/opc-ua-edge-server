...
docker cp opc-ua-edge-server:/root/.config/node-opcua-default-nodejs/PKI/own/certs/. . || exit 1
openssl x509 -inform PEM -outform DER -text -in certificate.pem -out opcua_cert.der || exit 1
mv opcua_cert.der /mount/pki/trusted/certs || exit 1
...
docker cp . opc-ua-edge-server:/root/.config/node-opcua-default-nodejs/PKI/trusted/certs || exit 1
